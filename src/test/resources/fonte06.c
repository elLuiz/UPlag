...
Skiplist *cria_skiplist(Skiplist *lista) {
    ...
    cabe->proximo = (no **) malloc(sizeof(no*) * (NIVEL_MAX + 1));
        ...
        //CRIA��O DE UM CABE�ALHO PARA TODOS OS N�VEIS DA SKIPLIST
...
int nivel_aleatorio() {
    ...
    //ESCOLHA ALEAT�RIA DE UM N�VEL ENTRE O PRIMEIRO E O N�VEL M�XIMO
...
//O FOR IRA PERCORRER OS NIVEIS ENQUANTO O WHILE PERCORRE AS LISTA NOS RESPECTIVOS NIVEIS
    for (i = lista->nivel; i >= 1; i--) {
        while (x->proximo[i]->chave < chave)
            x = x->proximo[i];
        aux_troca[i] = x; //RECEBE O �LTIMO N� MENOR QUE O N� QUE SE DESEJA INSERIR
    }
...
// CASO AS CHAVES SEJAM IGUAIS OCORRE A SUBST. DOS VALORES
    if (chave == x->chave) {
        ...
        //O NOVO N� � INSERIDO AP�S O AUX TROCA , DEFINIDO ANTERIORMENTE
        for (i = 1; i <= nivel; i++) {
...
no *busca_skiplist(Skiplist *lista, int chave) {
    ...
    no *x = lista->cabe;
    int i;
    //PERCORRE-SE OS NIVEIS E AS LISTAS AT� ENCONTRAR UMA CHAVE QUE NAO SEJA MENOR QUE A QUE PROCURAMOS
    for (i = lista->nivel; i >= 1; i--) {
        while (x->proximo[i]->chave < chave)
            x = x->proximo[i];
    }
    //VERIFICA SE A PROXIMA CHAVE,OU SEJA,A CHAVE QUE N�O � MENOR QUE A PROCURAMOS � IGUAL A ELA
    //SE SIM � RETORNAMOS SE N�O RETORNAMOS NULL
...
int remove_skiplist(Skiplist *lista, int chave) {
    ...
    //SE A CHAVE DA POSI��O X FOR IGUAL A QUE QUEREMOS REMOVER
    //FAZEMOS AS DEVIDAS ALTERA�OES USANDO O VETOR AUX_TROCA QUE CONTEM O NO ANTERIOR AO QUE DESEJAMOS REMOVER
    if (x->chave == chave) {
 ...
 void imprime_skiplist(Skiplist *lista) {
    ...
    //IMPRESS�O DE TODOS OS VALORES DA SKIPLIST , PERCORRENDO AS LISTAS EM TODOS OS NIVEIS
    for(i = lista->nivel ; i>= 1;i--){
...
//UTILIZANDO A MESMA ESTRAT�GIA DA IMPRESS�O, PERCORREMOS A SKIPLIST NO NIVEL 1 E AQUI IMPLEMETAMOS UM CONTADOR
...
//O FINAL DE CADA NIVEL DA SKIPLIST APONTA PARA O CABE�ALHO ENTAO SE O PROXIMO DO CABE�ALHO FOR O PROPRIO CABE�ALHO A LISTA ESTA VAZIA;
...
//PERCORRE-SE TODOS OS VALORES DA SKIPLIST E OS REMOVE UM A UM, LIBERANDO A MEM�RIA DE SEUS N�S
